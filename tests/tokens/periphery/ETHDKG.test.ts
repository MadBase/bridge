...
describe("ETHDKG", function () {
  ...
  describe("GPKj submission", () => {
    ...
    it("should not allow GPKj submission outside of GPKjSubmission phase", async () => {
      let [ethdkg] = await startAtMPKSubmission(
        validators4
      );

      await expect(ethdkg.connect(await ethers.getSigner(validators4[0].address)).submitGPKj(validators4[0].gpkj))
      .to.be.revertedWith("ETHDKG: Not in GPKJ submission phase")
    })

    it("should not allow non-validators to submit GPKj submission", async () => {
      let [ethdkg] = await startAtGPKJ(
        validators4
      );

      const validator11 = "0x23EA3Bad9115d436190851cF4C49C1032fA7579A";

      await expect(ethdkg.connect(await ethers.getSigner(validator11)).submitGPKj(validators4[0].gpkj))
      .to.be.revertedWith("ETHDKG: Only validators allowed!")
    })
    ...
    it("should not allow submission of GPKj more than once from a validator", async () => {
      ...
      await submitValidatorsGPKJ(ethdkg, validatorPool, validators4.slice(0,1), expectedNonce, 1)

      await expect(submitValidatorsGPKJ(ethdkg, validatorPool, validators4.slice(0,1), expectedNonce, 1))
      .to.be.revertedWith("Participant already submitted GPKj this ETHDKG round")
    })
  ...
  describe("Accuse participant of not submitting GPKj", () => {
    ...
    it("should not allow accusations related to missing GPKj submissions while on GPKj submission phase", async () => {
      ...
      // accuse one validator only
      await expect(ethdkg.accuseParticipantDidNotSubmitGPKJ([validators4[1].address]))
    ...
    it("should not allow GPKj submission after the GPKj submission phase", async () => {
      ...
      await expect(submitValidatorsGPKJ(ethdkg, validatorPool, validators4.slice(1,2), expectedNonce, 1))
      .to.be.revertedWith("ETHDKG: Not in GPKJ submission phase")
    ...
    it("should not allow missing validators to complete ETHDKG after the GPKj submission phase", async () => {
      ...
      await expect(ethdkg.connect(await ethers.getSigner(validators4[1].address)).complete())
      .to.be.revertedWith("ETHDKG: cannot complete yet")
    ...
    it("should not allow accusation of missing GPKj to a validator that actually submitted it", async () => {
      ...
      // accuse
      await expect(ethdkg.accuseParticipantDidNotSubmitGPKJ([validators4[0].address]))
      .to.be.revertedWith("Dispute failed! Issuer did participate in this GPKj submission!")
    ...
    it("should not allow accusation of missing GPKj to a non-existent validator", async () => {
      ...
      // accuse
      await expect(ethdkg.accuseParticipantDidNotSubmitGPKJ(["0x26D3D8Ab74D62C26f1ACc220dA1646411c9880Ac"]))
