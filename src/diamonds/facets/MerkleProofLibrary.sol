...
library MerkleProofLibrary {
    ...
    function computeLeafHash(bytes32 key, bytes32 value, uint16 proofHeight) internal pure returns (bytes32){
        require(proofHeight <= 256 && proofHeight >= 0, "MerkleProofLibrary: Invalid proofHeight, should be [0, 256[");
    ...
    function verifyNonInclusion(MerkleProofParserLibrary.MerkleProof memory _proof, bytes32 root) internal pure {
        if (_proof.proofKey == 0 && _proof.proofValue == 0) {
            ...
            bool result = checkProof(_proof.auditPath, root, _keyHash,  _proof.key, _proof.bitmap, _proof.keyHeight);
            require(result, "MerkleProofLibrary: The Leaf node provided was not found in the key's path!");
