...
contract("MadnetFactory", async (accounts) => {
    

    describe("MADNETFACTORY", async () => {
        before(async () => {
        ...
        it("setOwner: succeed", async () => {
            //sets the second account as owner 
            let receipt = await this.factory.setOwner(accounts[1], {from: ownerAccount});
            ownerAccount = accounts[1];
            expect(await this.factory.owner.call()).to.equal(ownerAccount);
            await this.factory.setOwner(accounts[0], {from: accounts[1]})
            ownerAccount = accounts[0]; 
            expect(await this.factory.owner.call()).to.equal(ownerAccount);
        });
    
        it("setDelegator: succeed", async () => {
            //sets the second account as delegator 
            await this.factory.setDelegator(accounts[1], {from: accounts[0]});
            delegatorAccount = accounts[1];
            expect(await this.factory.delegator()).to.equal(delegatorAccount);
        });
    
        it("SET OWNER WITH UNAUTHORIZED ACCOUNT EXPECT FAIL", async () => {
            await expectRevert(
                this.factory.setOwner(accounts[0], {from: delegatorAccount}),
                "unauthorized"
            );
        });
        ...
        it("DEPLOY MOCK WITH DEPLOYTEMPLATE AS OWNER EXPECT SUCCEED", async () => {
            ...
            let mockCon = await hre.ethers.getContractFactory("Mock");
            ...
            let transactionCount = await hre.ethers.provider.getTransactionCount(this.factory.address)
        ...
        it("DEPLOYSTATIC MOCK INITIALIZABLE CONTRACT", async () => {
            ...
            let byteCode = await getDeployTemplateArgs(mockInitializableKey);
            let receipt = await this.factory.deployTemplate(byteCode);
            let deployStatic = await getDeployStaticArgs(mockInitializableKey, [2])
            receipt = await this.factory.deployStatic(deployStatic.Salt, deployStatic.initCallData);
            let mockInitAddr = getEventVar(receipt, deployedStaticKey, contractAddrKey);
        ...
        it("DEPLOYPROXY EXPECT SUCCESS", async () => {
            ...
            this.proxySalt = hre.ethers.utils.formatBytes32String(salt.toString());
        ...
        it("DEPLOY PROXY WITH UNAUTH ACC EXPECT FAIL", async () => {
            await expectRevert(
                this.factory.deployProxy(this.proxySalt, {from: delegatorAccount}), 
                "unauthorized"
                );
        });
        ...
        it("DEPLOYCREATE MOCK LOGIC CONTRACT EXPECT SUCCESS", async () => {
            ...
            let expectedMockAddr = hre.ethers.utils.getContractAddress({
        ...
        it("DEPLOYCREATE MOCK LOGIC CONTRACT W/ UNAUTH ACC EXPECT FAIL", async () => {
            ...
            //deploy Mock Logic through the factory
            // 27fe1822
            let receipt = this.factory.deployCreate(deployBCode.data, {from: delegatorAccount});
        ...
        it("UPGRADE PROXY TO POINT TO DCMOCKADDRESS W FACTORY USING UNAUTH ACC EXPECT FAIL", async () => {
            let receipt = this.factory.upgradeProxy(this.proxySalt, this.dcMockAddress, {from: delegatorAccount});
            await expectRevert(receipt, "unauthorized")
        });
        ...
        it("DEPLOYCREATE WITH BAD CODE EXPECT FAIL", async () => {
            let receipt = this.factory.deployCreate("0x6000", {from: ownerAccount});
        ...
        it("DEPLOYCREATE2 MOCKINITIALIZABLE", async () => {
            let receiptObj = await deployCreate2Initable(this.factory);
            let receipt = receiptObj[receiptKey];
            expectTxSuccess(receipt);
        ...
        it("INITIALIZE MOCK CONTRACT", async () => {
            let mockInitable = await hre.ethers.getContractFactory(mockInitializableKey);
        ...
        it("CALLANY", async () => {
            ...
            let mockInitable = await hre.ethers.getContractFactory(mockInitializableKey);
            let initCallData = await mockInitable.interface.encodeFunctionData("initialize", [2]);
            receipt = await this.factory.callAny(mockInitAddr, 0, initCallData);
            await checkMockInit(mockInitAddr, 2)
        });
        it("DELEGATECALLANY", async () => {
            //deploy an instance of mock logic for factory 
            let mockFactoryInstance = await mockFactory.new();
            //generate the call data for the factory instance 
            let mfEncode = await hre.ethers.getContractFactory("MockFactory");
            ...
            let receipt = await this.factory.delegateCallAny(mockFactoryInstance.address, setOwner);
            expectTxSuccess(receipt);
            let owner = await this.factory.owner.call();
            expect(owner).to.equal(accounts[2]);
            setOwner = await mfEncode.interface.encodeFunctionData("setOwner", [accounts[0]]);
            receipt = await this.factory.delegateCallAny(mockFactoryInstance.address, setOwner, {from: accounts[2]});
            expectTxSuccess(receipt);
            owner = await this.factory.owner.call();
        ...
        describe("MULTICALL DEPLOY PROXY", async () => {
            it("MULTICALL DEPLOYPROXY, DEPLOYCREATE, UPGRADEPROXY EXPECT SUCCESS", async () => {
                ...
                salt = salt.getTime();
                //get the utf8 bytes32 version of the salt 
                let Salt = hre.ethers.utils.formatBytes32String(salt.toString());
                let mockCon = await hre.ethers.getContractFactory("Mock");
                //deploy code for mock with constructor args i = 2
                deployBCode = mockCon.getDeployTransaction(2);
                const MadnetFactory = await hre.ethers.getContractFactory("MadnetFactory");
                let transactionCount = await hre.ethers.provider.getTransactionCount(this.factory.address);
            ...
            it("MULTICALL DEPLOYPROXY, DEPLOYTEMPLATE, DEPLOYSTATIC, UPGRADEPROXY, EXPECT SUCCESS", async () => {
                ...
                //get the utf8 bytes32 version of the salt
                //salt for proxy 
                let proxySalt = hre.ethers.utils.formatBytes32String(salt1.toString());
                let mockCon = await hre.ethers.getContractFactory("Mock");
                let salt2 = new Date();
                salt2 = salt2.getTime();
                //salt for deployStatic
                let metaSalt = hre.ethers.utils.formatBytes32String(salt2.toString());
                //deploy code for mock with constructor args i = 2
                deployBCode = mockCon.getDeployTransaction(2);
    ...
    describe("MADNETFACTORY API TEST", async () => {
        ...
        it("DEPLOYUPGRADEABLE", async () => {
            let res = await deployUpgradeable("Mock", this.factory.address, [2]);
            let cSize = await this.utils.getCodeSize(res.logicAddress)
            expect(cSize.toNumber()).to.be.greaterThan(0);
            cSize = await this.utils.getCodeSize(res.proxyAddress)
            expect(cSize.toNumber()).to.be.greaterThan(0);
        });
    
        it("UPGRADEDEPLOYMENT", async () => {
            let res = await upgradeProxy(madnetFactoryKey, this.factory.address);
            let logicSize = await this.utils.getCodeSize(res.logicAddress)
            expect(logicSize.toNumber()).to.be.greaterThan(0);
        });
    
        it("DEPLOYSTATIC", async () => {
            let res = await deployStatic(endPointKey, this.factory.address);
            let cSize = await this.utils.getCodeSize(res.templateAddress)
            expect(cSize.toNumber()).to.be.greaterThan(0);
            cSize = await this.utils.getCodeSize(res.metaAddress)
            expect(cSize.toNumber()).to.be.greaterThan(0);
        });
    });
...
function bytes32ArrayToStringArray(bytes32Array){
    ...
    for(let i = 0; i < bytes32Array.length; i++){
        ret.push(hre.ethers.utils.parseBytes32String(bytes32Array[i]));
    }
    return ret;
}
...
function getSalt(){
     ...
     //use the time as the salt 
     salt = salt.getTime();
     return hre.ethers.utils.formatBytes32String(salt.toString());
}
...
async function checkMockInit(target, initVal){
    let mock = await mockInit.at(target);
    let i = await mock.i.call();
    expect(i.toNumber()).to.equal(initVal);
}
async function deployFactory(factoryName){
    let factoryInstance = await hre.artifacts.require(factoryName);
    //gets the initial transaction count for the address 
    let transactionCount = await hre.ethers.provider.getTransactionCount(ownerAccount);
...
async function deployCreate2Initable(factory){
     ...
     let ret = {};
     //get the utf8 bytes32 version of the salt 
     ret["Salt"] = hre.ethers.utils.formatBytes32String(salt.toString());
     ret["receipt"] = await factory.deployCreate2(0, ret["Salt"], mockInit.bytecode);
     return ret;
}
