...
contract CryptoTest is DSTest {
    ...
    /// "call"s "Sign(bytes,uint256)" on the crypto member contract. Converts response "bytes" into expected "uint256[2]"
    function wrappedSign(bytes memory message, uint256 privateKey) internal returns (bool, uint256[2] memory) {
        bool ok;
        bytes memory res;
        (ok, res) = address(crypto).call(abi.encodeWithSignature("Sign(bytes,uint256)", message, privateKey)); // solium-disable-line
        
        uint256[2] memory signed;
        if (ok) {
            signed[0] = sliceUint(res, 0);
            signed[1] = sliceUint(res, 32);
        }

        return (ok, signed);
    }

    function wrappedAggregateSignatures(uint256[2][] memory sigs, uint256[] memory indices, uint256 threshold, uint256[] memory invArray) internal returns (bool, uint256[2] memory) {
        // AggregateSignatures(uint256[2][] memory sigs, uint256[] memory indices, uint256 threshold, uint256[] memory invArray) public view returns (uint256[2] memory)
        bool ok;
        bytes memory res;
        (ok, res) = address(crypto).call(
            abi.encodeWithSignature("AggregateSignatures(uint256[2][],uint256[],uint256,uint256[])", sigs, indices, threshold, invArray)); // solium-disable-line

        uint256[2] memory aggregate;
        if (ok) {
            aggregate[0] = sliceUint(res, 0);
            aggregate[1] = sliceUint(res, 32);
        }
    ...
    function testSign() public {
        ...
        bool ok;
        uint256[2] memory signed;
        (ok, signed) = wrappedSign(message, privateKey);
    ...
    function testVerify() public {
        ...
        bytes memory res;
        (ok, res) = address(crypto).call(abi.encodeWithSignature("Verify(bytes,uint256[2],uint256[4])", message, signed, publicKey)); // solium-disable-line
        assertTrue(ok);
        assertEq(res.length, 32);

        uint256 fakeBool = sliceUint(res, 0);
        assertEq(fakeBool, 1);
    }

    function testVerifyNegative() public {
        // Verify(bytes memory message, uint256[2] memory sig, uint256[4] memory pubK) public view returns (bool v)
        bytes memory message = bytes(hex"00010203");
        uint256 privateKey = 1234567890;
    ...
    function testHashToG1() public {
        ...
        bool ok;
        bytes memory res;
        (ok, res) = address(crypto).call(abi.encodeWithSignature("HashToG1(bytes)", message)); // solium-disable-line

        assertTrue(ok);
        assertEq0(res, bytes(hex"1b319b4cc4d718c6294de4e1704c988ba22489719e908af33db90969875aaa1b" 
                             hex"21e3ec9b261e303e0ace5ecc93b9db53b88a7fe0e9d082747965a8a559fe752b"));
    }
    ...
    function testSafeSigningPoint() public {
        ...
        bool safe = crypto.safeSigningPoint(validPoint);

        assertTrue(safe);
    }
    
    function testSafeSigningPointNegative() public {
        // safeSigningPoint(uint256[2] memory input) public pure returns (bool)

        uint256[2] memory infinity; // elements default to 0, which is exactly what we want

        bool safe = crypto.safeSigningPoint(infinity);
    ...
    function testAggregateSignatures() public {
        ...
        bool ok;
        uint256[2] memory aggregate;
        (ok, aggregate) = wrappedAggregateSignatures(signatures, indices, threshold, inverses);
    ...
    function testAggregateSignaturesNegativeLengthMismatch() public {
    ...
    function testAggregateSignaturesNegativeThreshold() public {
        uint256[2][] memory signatures = getSignatures(2);
